...
const char* runtime_error_msg = "Error initializing recorder";
...
void recorder<CODEC_ID>::init()
{
    ...
    {
        if(avio_open2(&m_out_format_ctx->pb, outfile_path, AVIO_FLAG_WRITE, nullptr, nullptr) < 0)
            throw std::runtime_error {runtime_error_msg};
    }
    ...
    av_dump_format(m_out_format_ctx, 0, outfile_path, 1);
